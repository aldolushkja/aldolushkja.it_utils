...
function ensureRoute(route) {
  ...
  if (bundle) {
    if (isString(bundle)) {
      ...
      throw new Error(
          log('Expected route bundle to include either "' + NOMODULE + '" or "'
              + MODULE + '" keys, or both')
      );
    } else {
...
function tokensToFunction(tokens) {
  ...
  return function (data, options) {
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      throw new TypeError(
          'Expected "' + token.name + '" to be ' + (token.repeat ? 'an array'
          : 'a string'))
    }
...
function tokensToRegExp(tokens, keys, options) {
  ...
  var endsWith = [].concat(options.endsWith || []).map(escapeString).concat(
      '$').join('|');
  var route = start ? '^' : '';
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    if (typeof token === 'string') {
      ...
      isEndDelimited = i === tokens.length - 1 && delimiters.indexOf(
 ...
 */
function matchRoute(route, pathname, ignoreLeadingSlash, parentKeys,
    parentParams) {
  let match;
  let childMatches;
  let childIndex = 0;
  ...
  return {
    next(routeToSkip) {
      ...
      if (!match) {
        match = matchPath(routepath, pathname, !children, parentKeys,
            parentParams);
  ...
  if (route && !route.__synthetic) {
    ...
    if (!context.chain) {
      ...
      if (route.parent) {
        ...
        while (i-- && context.chain[i].route && context.chain[i].route
        !== route.parent) {
          context.chain.pop();
        }
      }
...
class Resolver {
  ...
  resolve(pathnameOrContext) {
    const context = Object.assign(
        {},
        this.context,
        isString(pathnameOrContext) ? {pathname: pathnameOrContext}
            : pathnameOrContext
    );
...
function cacheRoutes(routesByName, route, routes) {
  ...
  if (Array.isArray(routes)) {
    for (let i = 0; i < routes.length; i++) {
      ...
      childRoute.parent = route;
      cacheRoutes(routesByName, childRoute,
...
function amend(amendmentFunction, args, element) {
  return amendmentResult => {
    if (amendmentResult && (amendmentResult.cancel
...
function getPathnameForRouter(pathname, router) {
  ...
  return base
      ? router.constructor.__createUrl(pathname.replace(/^\//, ''),
          base).pathname
      : pathname;
}
...
class Router extends Resolver {
  ...
  constructor(outlet, options) {
    ...
    super([], Object.assign({
      // Default options
      baseUrl: baseHref && Resolver.__createUrl(baseHref,
          document.URL).pathname.replace(/[^\/]*$/, '')
    }, options));

    this.resolveRoute = context => this.__resolveRoute(context);

    const triggers = Router.NavigationTrigger;
    Router.setTriggers.apply(Router,
    ...
    .then(result => {
      ...
      if (route.bundle) {
        ...
        .then(() => {
          ...
          throw new Error(
              log(`Bundle not found: ${route.bundle}. Check if the file name is correct`));
        });
      }
    })
    .then(result => {
    ...
    .then(context => {
      if (this.__isLatestRender(context)) {
        ...
        fireRouterEvent('location-changed',
            {router: this, location: this.location});
  ...
  __fullyResolveChain(topOfTheChainContextBeforeRedirects,
    ...
    .then(contextAfterRedirects => {
      const redirectsHappened = contextAfterRedirects
          !== contextBeforeRedirects;
      const topOfTheChainContextAfterRedirects =
          redirectsHappened ? contextAfterRedirects
      ...
      const findNextContextIfAny = (context, parent = context.route,
          prevResult) => {
      ...
      return findNextContextIfAny(contextAfterRedirects).then(nextContext => {
        ...
        && nextContext !== contextAfterRedirects
            ? this.__fullyResolveChain(topOfTheChainContextAfterRedirects,
                nextContext)
  ...
  __amendWithOnBeforeCallbacks(contextWithFullChain) {
    return this.__runOnBeforeCallbacks(contextWithFullChain).then(
        amendedContext => {
          if (amendedContext === this.__previousContext || amendedContext
              === contextWithFullChain) {
      ...
      if (newContext.__skipAttach) {
        ...
        for (let i = newChain.length - 1; i >= 0; i--) {
          callbacks = this.__runOnBeforeLeaveCallbacks(callbacks, newContext,
              {prevent}, previousChain[i]);
        }
        // execute onBeforeEnter for changed segment element when skipping attach
        for (let i = 0; i < newChain.length; i++) {
          callbacks = this.__runOnBeforeEnterCallbacks(callbacks, newContext,
              {prevent, redirect}, newChain[i]);
          previousChain[i].element.location = createLocation(newContext,
              previousChain[i].route);
        }

      } else {
        // execute onBeforeLeave when NOT skipping attach
        for (let i = previousChain.length - 1;
            i >= newContext.__divergedChainIndex; i--) {
          callbacks = this.__runOnBeforeLeaveCallbacks(callbacks, newContext,
              {prevent}, previousChain[i]);
        }
      }
      ...
      for (let i = 0; i < newChain.length; i++) {
        if (i < newContext.__divergedChainIndex) {
          ...
          if (newChain[i].element) {
            newChain[i].element.location = createLocation(newContext,
                newChain[i].route);
          }
        }
      }
      ...
      if (amendmentResult) {
        ...
        if (amendmentResult.redirect) {
          return this.__redirect(amendmentResult.redirect,
              newContext.__redirectCount, newContext.__renderId);
        }
      }
      return newContext;
    });
    ...
    return callbacks.then(result => {
      if (this.__isLatestRender(newContext)) {
        const afterLeaveFunction = amend('onBeforeLeave',
            [location, commands, this], chainElement.element);
  ...
  __isReusableElement(element, otherElement) {
    if (element && otherElement) {
      return this.__createdByRouter.get(element) && this.__createdByRouter.get(
          otherElement)
  ...
  __updateBrowserHistory({pathname, search = '', hash = ''}, replace) {
    if (window.location.pathname !== pathname
      ...
      window.history[changeState](null, document.title,
  ...
  __copyUnchangedElements(context, previousContext) {
    ...
    for (let i = 0; i < context.__divergedChainIndex; i++) {
      const unchangedElement = previousContext
          && previousContext.chain[i].element;
      if (unchangedElement) {
  ...
  __addAppearingContent(context, previousContext) {
    ...
    const deepestCommonParent = this.__copyUnchangedElements(context,
        previousContext);
  ...
  __runOnAfterLeaveCallbacks(currentContext, targetContext) {
    ...
    for (let i = targetContext.chain.length - 1;
        i >= currentContext.__divergedChainIndex; i--) {
  ...
  __runOnAfterEnterCallbacks(currentContext) {
    ...
    for (let i = currentContext.__divergedChainIndex;
        i < currentContext.chain.length; i++) {
      if (!this.__isLatestRender(currentContext)) {
        break;
      }
...
const FlowClients = window.Vaadin && window.Vaadin.Flow
...
function uncommentAndRun(callback, args) {
  ...
  if (match) {
    try {
      ...
      // eat the exception
      console.log('vaadin-development-mode-detector: uncommentAndRun() failed',
          e);
    }
  }
